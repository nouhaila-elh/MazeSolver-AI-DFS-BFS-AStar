...
public class AbstractSearchEngine {
    ...
    protected Dimension [] getPossibleMoves(Dimension loc) {
        ...
        int y = loc.height;
        int num = 0;
        if(maze.getValue(x - 1, y) == 0 || maze.getValue(x - 1, y) == -5 || maze.getValue(x - 1, y) == -4 || maze.getValue(x - 1, y) == Maze.GOAL_LOC_VALUE){
        ...
        if (maze.getValue(x, y - 1) == 0 || maze.getValue(x, y - 1) == -5 || maze.getValue(x, y - 1) == -4 || maze.getValue(x, y - 1) == Maze.GOAL_LOC_VALUE) {
